...
import com.zp.library_base.preference.BasicDataPreferenceUtil;
...
public abstract class SuperBaseModel<T> {
    ...
    /**
     * 作用：获取数据-从本地缓存或网络
     * 使用：提供给VM实现类通过new的model实例调用该方法启动数据获取
     */
        ...
        if (getCachedPreferenceKey() != null) {
            ...
            if (!TextUtils.isEmpty(saveDataString)) {
                try {
                    ...
                    if (savedData != null) {
                        //如果缓存的数据有值先加载缓存数据，不至于页面出现加载到数据前的空白
